...
'''
Script to simulate rain in a square field. Counting the number of rain drops in the inscribed circle of radius equal to 
the length of the field. The ratio of the number of drops in the circle to the total number of drops gives pi
credit: https://github.com/drvinceknight/Monte_Carlo_Simulation_for_Estimating_pi/blob/master/Estimate_pi.py
'''


def rain_drop(length_of_field=1):
    """
    Simulate random raindrop
    """
...
def plot_rain_drops(drops_in_circle, drops_out_of_circle, length_of_field=1, format='pdf'):
    ...
    plt.savefig("%s drops.%s" % (number_of_drops, format))
...
def rain(number_of_drops=1000, length_of_field=1, plot=True, format='pdf', dynamic=False):
    ...
    for k in range(number_of_drops):
        ...
        if dynamic:  # The dynamic option if set to True will plot every new drop (this can be used to create animations
            # in the simulation)
            print("Plotting drop number: %s" % (k + 1))
            plot_rain_drops(drops_in_circle, drops_out_of_circle, length_of_field, format)
        pi_estimate.append(4 * number_of_drops_in_circle / (k + 1))  # Updates list with the newest estimate for pi.
...
if __name__ == "__main__":
    # Run script from cli (don't know what this means)
    ...
    # Two sets of sims (comment out unwanted one)
