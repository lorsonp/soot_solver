...
def initiate_system():
    ...
    return particles, N, sample_times, running_time, stop_time

def main(particles, N, sample_times, running_time, stop_time):
    ...
    while running_time < stop_time:
        ...
        # ============================================ #
        time_steps = []
        inception_rate = get_inception_rate(1900, 100000)
...
def get_sample_times(start_time, stop_time):
    sample_times = np.arange(start_time, stop_time, 1)
    return sample_times
...
def coagulation_step(particles, N):
    ...
    if r <= coag_kernel/maj_kernel:
        ...
        group = np.random.randint(len(particles))
        particles[group][index_1 + index_2] += 1
...
# Run Simulation
[particles, N, sample_times, running_time, stop_time] = initiate_system()
main(particles, N, sample_times, running_time, stop_time)
